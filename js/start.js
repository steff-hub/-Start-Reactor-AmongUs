startReactor = {
    ...
    computerCombination: [],
    playerCombination: [], // verificar se a sequencia esta correta.
    computerCombinationPosition: 1, //verificar a posição 
    computerMaxPosition: 5, 
    memoryMaxCombination: 9, //verificar
    ...
    audio: {
        ...
        loadAudio(filename) {
            //desse jeito ele tranforma a string em um audio, ele ecarrega o objeto e retorna para o atributo 

            const file = `./audio/${filename}?cb=${new Date().getTime()}`// o parametro e para forçar ele n ficar no cache.
            const audio = new Audio(file) // objeto de audio
            audio.load()
            return audio
        ...
        loadAudios() {
            ...
            startReactor.audio.combinations = startReactor.audio.combinations.map ( (audio) => startReactor.audio.loadAudio(audio)) // como é um array de combinações então voce consegue fazer um map 
    ...
    interface: {
        ...
        playerMemoryButtons: document.getElementsByClassName("player_memory"),// não foi usado um metodo para se fazer enjeção!

        turnLedOn(index, ledPanel) { //controle de time
            ledPanel.children[index].classList.add("ledOn");// reponsavel por deixar o led acesso 
        },

        turnAllLedsOff() {
           // apagar todos os leds no final da partida 
        ...
        async start() {
            return startReactor.audio.start.play() // retornou o load do audio
        },
    ...
    createCombination() {
        //ela vai fazer uma randonização entre o zero e o memoryMaxCombination
          let newCombination = []
